...
test_data_path = '../data'
tomo_test_data_file = test_data_path + f'/MA_tomo_tau_dm_30000_qs_2_alpha_TEST_0.1.pickle'
...
for i in range(len(kparams_values)):
    plt.errorbar(bin_means, means[i], yerr=stds[i], xerr=bin_stds, fmt=f'--{colors[i]}o', ecolor=f'{colors[i]}',
                 ...
                 xlolims=False, xuplims=False, errorevery=1, capthick=1, data=None, label=f'K = {i+4}')
...
# plt.legend()
plt.xticks(np.arange(0.9,1.1,0.05), fontsize=fs)
plt.yticks(np.arange(0.94, 1.1,0.02), fontsize=fs)
#
...
plt.subplots_adjust(bottom=0.6, left=0.6)
#
plt.savefig('fid_vs_purity_MA_MA_alpha_0.1_zoomed_crossing_point.svg', dpi=600)
