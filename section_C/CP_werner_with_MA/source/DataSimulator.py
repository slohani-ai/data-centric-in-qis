...
class Werner_Two_Q:
    ...
    def rho_werner(self, n_size):
        ...
        eta = tf.random.uniform([n_size, 1, 1], minval=self.eta[0], maxval=self.eta[1])
        frac = (1. - eta) / 4.
...
class Separable_States:
    ...
    def __init__(self, qs, pure=True):
        self._qs = qs
        self.purity = pure

    def sep_state(self, _):
        if self.purity:
            states = qi.random_density_matrix(dims=2, rank=1).tensor(qi.random_density_matrix(dims=2, rank=1))
        else:
            states = qi.random_density_matrix(dims=2).tensor(qi.random_density_matrix(dims=2))
        return states.data
...
class Measurements_Shots:
    ...
    def measurement_array(self, dm=None):
        dm = np.squeeze(dm)
        proj_rank = np.tile(np.arange(9), self.n_shots)
