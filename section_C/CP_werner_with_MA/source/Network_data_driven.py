...
import data_loader_data_driven as dl
import numpy as np
...
class ARL_Network(object):
    ...
    def Network(self,
        ...
        # mid_dense = tf.keras.layers.Dense(units=2000,
        #                               activation=act_fn,
        #                               name='mide_DENSE')(dropout_1)
        # dropout_mid = tf.keras.layers.Dropout(0.5, name='Mid_DROPOUT')(mid_dense)

        dense_2 = tf.keras.layers.Dense(units=self._dense_2,
                                        activation=act_fn,
                                        name='Sec_DENSE') (dropout_1) #(dropout_mid) # (dropout_1) # (dropout_mid)
    ...
    def Run_Network(self, epoch_n=20, eta=0.01, train_n=15000, valid_n=50, from_checkpoint=False,
                    ens_rank=1, trial=1, test_data_path='data_path', train_data_path='train_data_path',
                    save_folder_path='path_to_save', kparams=4, n_add_separable_sets=None):
        ...
        dm_valid = dl.loader(out_class=self._outclass,
                                      ...
                                      kparams = kparams,
                                      n_add_separable_sets=n_add_separable_sets)
        ...
        for epoch in range(epochs):
            ...
            if (epoch+1) % 25 == 0:
                ...
                if epoch > 20 and av > np.max(av_fid):
                    ...
                    model.save(f'{save_folder_path}/data_driven_training_{train_n}_dense_1_{self._dense_1}_dense_2_{self._dense_2}_alpha_{ens_rank}_k_params_{kparams}_trial_{trial}_n_add_separable_sets_{n_add_separable_sets}_BEST.h5')
        ...
        with open(f'./misc_outputs/data_driven/rank_distributions_per_5_epochs_qubits_{self._qs}_trial_{trial}_training_{train_n}_dense_1_{self._dense_1}_dense_2_{self._dense_2}_alpha_{ens_rank}_k_params_{kparams}_n_add_separable_sets_{n_add_separable_sets}_.pkl','wb') as f:
            pkl.dump(rank_pred_list, f, -1)
        with open(f'./misc_outputs/data_driven/Total Training_time_qubits_{self._qs}_trial_{trial}_training_{train_n}_dense_1_{self._dense_1}_dense_2_{self._dense_2}_alpha_{ens_rank}_k_params_{kparams}_n_add_separable_sets_{n_add_separable_sets}_.pkl', 'wb') as f:
            pkl.dump(end - start, f, -1)
        print(end - start)
        with open(f'./misc_outputs/data_driven/fidelity_list_qubits_{self._qs}_trial_{trial}_training_{train_n}_dense_1_{self._dense_1}_dense_2_{self._dense_2}_alpha_{ens_rank}_k_params_{kparams}_n_add_separable_sets_{n_add_separable_sets}_.pkl','wb') as f:
            pkl.dump(av_fid,f,-1)
        # model.save('ARL_model_{}_batch_4_no_noise.h5'.format('60K_pure_states'))
        model.save(f'{save_folder_path}/data_driven_training_{train_n}_dense_1_{self._dense_1}_dense_2_{self._dense_2}_alpha_{ens_rank}_k_params_{kparams}_trial_{trial}_n_add_separable_sets_{n_add_separable_sets}_LAST.h5')
